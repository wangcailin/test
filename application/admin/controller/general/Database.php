...
class Database extends Backend
{
    ...
    public function query()
    {
        ...
        if (in_array($do_action, array('doquery', 'optimizeall', 'repairall', 'export_excel', 'export_word')))
        {
            $this->$do_action();
        }
    ...
    {
        $do_sql = $this->request->post('do_sql');
        $res = Db::query($do_sql);
        if ($res){
            $comlon = array_keys($res[0]);
        }

        export_excel($res, $comlon, 'tableName-'.date('Y.m.d'));

    }

    public function export_word($sql = null)
    {
        if ($sql == '')
            exit(__('SQL can not be empty'));
