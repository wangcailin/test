...
class Ajax extends Backend
{
        ...
        switch ($type) {
            ...
            case 'addons' || 'all':
                ...
                if ($type == 'addons')
                    break;
            case 'importfile' || 'all':
                rmdirs(RUNTIME_PATH . 'uploadfile' . DS, false);
                if ($type == 'importfile')
                    break;
        }
    ...
    {
        $where = ['status' => 'normal'];
        $pojectlist = Db::name('project_list')->where($where)->field('id as value,name')->order('id desc')->select();
        $this->success('', null, $pojectlist);
    }
    ...
    public function module()
    {
        ...
        if ($projectid !== '') {
            ...
            $modulelist = Db::name('project_module')->where($where)->field('id as value,name')->order('id desc')->select();
        }
        $this->success('', null, $modulelist);
    }
